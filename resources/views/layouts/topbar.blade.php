    ...
    <div class="layout-width">
        <div class="navbar-header">
            ...
            <div class="d-flex align-items-center">
                ...
                @if (Auth::user())
                    @php ($positions = \App\Models\Position::where('user_id', Auth::user()->id)->where('order_id', null)->get())
                    <div class="dropdown topbar-head-dropdown ms-1 header-item">
                        <button type="button" class="btn btn-icon btn-topbar btn-ghost-secondary rounded-circle"
                            ...
                            <span
                                class="position-absolute topbar-badge fs-10 translate-middle badge rounded-pill bg-info">{{ count($positions) }}<span
                        ...
                        <div class="dropdown-menu dropdown-menu-xl dropdown-menu-end p-0"
                            ...
                            <div class="p-3 border-top-0 border-start-0 border-end-0 border-dashed border">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-auto">
                                        <span class="badge badge-soft-warning fs-13">{{ count($positions) }} предметом</span>
                                    </div>
                                </div>
                            </div>
                            <div data-simplebar style="max-height: 300px;">
                                ...
                                <div class="p-1">
                                    <div class="d-block dropdown-item text-wrap px-3 py-2">
                                        <div class="d-flex align-items-center">
                                            <img src="{{ URL::asset('images/' . $position->item->image) }}"
                                                 class="me-3 rounded-circle avatar-sm p-2 bg-light" alt="user-pic">
                                            <div class="flex-1">
                                                <h6 class="mt-0 mb-1 fs-14">
                                                    <a href="{{ route('item', $position->item->id) }}" class="text-reset">
                                                        {{ $position->item->name }}
                                                    </a>
                                                </h6>
                                                <p class="mb-0 fs-12 text-muted">
                                                    Количество: <span>{{ $position->amount }} x {{ $position->item->cost }}р</span>
                                                </p>
                                            </div>
                                            <div class="px-2">
                                                <h5 class="m-0 fw-normal">{{ $position->amount * $position->item->cost }}р</h5>
                                            </div>
                                            <div class="ps-2">
                                                <form method="POST" action="{{ route('basket.remove', $position->id) }}">
                                                    @csrf
                            ...
                            <div class="p-3 border-bottom-0 border-start-0 border-end-0 border-dashed border d-grid">
                                <div class="d-flex justify-content-between align-items-center pb-3">
                                    ...
                                    <div class="px-2">
                                        <h5 class="m-0">{{ $summ }}р</h5>
                                    </div>
                                </div>

                                <a href="{{ route('orders.create') }}" class="btn btn-success text-center">
                                    Заказать
                                </a>
                            </div>
                        </div>
